...
function SetupProjectD9(args) {
  this.args = args;
  this.args.cms = 'D9';

  this.SetupProjectSetup = () => {
    log(chalk.yellow("Starting SetupProjectSetup D9 operations"), 0);
    ...
    log(chalk.yellow("Copied: drush_config_import.php"), 0);

    fileTpl = __dirname + "/src/new_relic_deploy.php";
    file = "./web/private/new_relic_deploy.php";
    fsx.copySync(fileTpl, file);
    log(chalk.yellow("Copied: new_relic_deploy.php"), 0);
    ...
    log(chalk.yellow(".gitignore updated"), 0);
    ...
    composerFile.scripts['phpunit-test'] = "./.ci/scripts/tests/on-server-phpunit-tests.sh";

    // phpunit/phpunit
    // if (!composerFile['require-dev']) {
    //   composerFile['require-dev'] = {};
    // }
    // composerFile['require-dev']['phpunit/phpunit'] = "^8";
    ...
    composerFile.extra['installer-types'] = ["custom-drupal-module"];
    composerFile.extra['installer-paths']["web/modules/bluecadet/{$name}"] = ["type:custom-drupal-module"];
    ...
    let composer = new Composer({ cms: "D9", packages: ['adminTheme', 'devModules', 'baseBluecadet', 'baseWeb'] });
    composer.run();
