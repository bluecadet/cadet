...
module.exports.initLocalDbCreds = () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const setup = [
      ...
      {
        type: 'input',
        query: `${chalk.cyan('Number (in minutes) that an existing backup is valid for (if backup is older, a new one will be created)')} 90]:`,
        handle: 'bac_exp'
      },
    ];
    ...
    return qoa.prompt(setup).then((response) => {
      ...
      creds.bac_exp = response.bac_exp ? response.bac_exp : 90;
