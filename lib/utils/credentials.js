...
cred_config.dbCredsFileName = "dbCreds.json";
...
module.exports.initGithubCreds = () => {
  return new Promise((resolve, reject) => {
    ...
    const gitMachineTokenPrompt = [
      {
        type: 'input',
        query: `${chalk.cyan('Github machine token (or `n` to skip to username and password):')}`,
...
module.exports.initLocalDbCreds = () => {
  return new Promise((resolve, reject) => {
    ...
    const setup = [
      ...
      {
        type: 'input',
        query: `${chalk.cyan('Local database password')} []:`,
        handle: 'db_pass'
      },
      {
        type: 'input',
        query: `${chalk.cyan('Local database tabel prefix')} []:`,
        handle: 'db_prefix'
      },
      {
        type: 'input',
    ...
    return qoa.prompt(setup).then((response) => {
      ...
      creds.db_pass = response.db_pass ? response.db_pass : '';
