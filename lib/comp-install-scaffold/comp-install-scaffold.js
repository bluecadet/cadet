...
const compPackages = require('./packages/packages.js').COMPOSER_PACKAGES;
            ...
            if (args.packages) {
              args.packages.forEach(el => {
                ...
                if (compPackages[args.cms] && compPackages[args.cms][el]) {
                  log(chalk.yellow("Adding packages: " + compPackages[args.cms][el].join(" ")), 1);
                  composerPack = spawnSync('composer', ['require', '--no-update'].concat(compPackages[args.cms][el]), {
                    stdio: 'inherit' // Will use process .stdout, .stdin, .stderr
                  });
