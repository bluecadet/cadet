    ...
    switch (args.cms) {
      ...
      case "WP":
        ...
        try {
            ...
            if (args.packages) {
              ...
              args.packages.forEach((bundle) => {
                ...
                if (compPackages[args.cms] && compPackages[args.cms][bundle]) {
                  log(chalk.blue(`...adding additional packages...`), 0);
          ...
          try {
            log(chalk.blue("...`composer install` one more time..."), 0);
          ...
          try {
            log(chalk.green("...trying to activate plugins with WP CLI..."), 0);
                ...
                args.packages.forEach((bundle) => {
                  ...
                  if (compPackages[args.cms] && compPackages[args.cms][bundle]) {
                    compPackages[args.cms][bundle].wp_cli.forEach(plugin => {
                      ...
                      let activatePlugin = spawnSync('wp', ['plugin', 'activate', plugin], {
                        // stdio: 'inherit' // Will use process .stdout, .stdin, .stderr
                      });
                      // No WP CLI
                      if (activatePlugin.status !== 0) {
                        log(`  - ${plugin}:` + chalk.green(' not activated'));
                      } else {
                        log(chalk.yellow(`  + ${plugin}`).italic('activated'));
                      }
                    });
                  }
                });
