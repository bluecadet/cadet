 ...
 * @param {object} site
 */
 function createPantheonBackup(site, args = '--element=db') {

  return new Promise((resolve, reject) => {
    exec(`terminus backup:create ${args} --keep-for=1 ${site}`, (err, stdout, stderr) => {
      if (err) {
        reject();
        console.log(chalk.red(`ERROR @ terminus backup:create ${args}  ${site}`));
        throw Error(err);
      }
 ...
 */
 function getPantheonDatabaseBackupURL(site, bac_exp, args = '--element=db') {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (bac_exp === 'override') {
      ...
      createPantheonBackup(site, args)
        .catch(err => {
          throw Error(err);
        })
        .then(url => {
 ...
 *
 */
function getPantheonEnvsArray(siteName) {
 ...
 *
 */
function getPantheonDB(site, dbConfig) {
 ...
 */
function getPantheonFiles(site, dbCreds) {
