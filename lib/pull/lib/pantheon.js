 ...
 */
 function createPantheonBackup(site) {
  ...
  return new Promise((resolve, reject) => {
    exec(`terminus backup:create --element=db --keep-for=1 ${site}`, (err, stdout, stderr) => {
      if (err) {
        reject();
...
/**
 * Get Specific File or Directory from Pantheon
 */
exports.getPantheonSpecificFiles = (site, dbCreds, req_path_data) => {
  getPantheonConnectionData(site)
    .then(data => {
      ...
      const rtype = req_path_data.file? 'f' : 'd';
      let req_path = req_path_data.file ?? req_path_data.dir;
      let path_data = path.parse(req_path);

      let src = `${creds.sftp_username}@${creds.sftp_host}:files/${req_path}`;
      let dest = `${FILES_PATH}/`;


      // if no extension, assume a directory.
      if (rtype === 'd') {

        dest = path.join(dest, req_path);
        if (req_path.slice(-1) !== path.sep) {
          dest += path.sep;
          src += path.sep;
        }
        ...
        dest = path.join(dest, path.dirname(req_path)) + "/";
      }
